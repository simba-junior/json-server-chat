...
// const io = new Server(server, {
//   cors: {
//     origin: ['http://localhost:1234'], // в случае КОРС ошибок вводить сюда тот локалхост на котором развернут фронт
//     methods: ['GET', 'POST'],
//   },
// });
const userRouter = jsonServer.router(join(__dirname, 'db/db.json'))
const swaggerFile = JSON.parse(readFileSync(join(__dirname, 'swagger/output.json'), 'utf-8'))
...
io.on('connection', (socket) => {
  ...
  socket.on('join_channel', async (data) => {
    ...
    try {
      ...
      const allMessages = await fetch(`http://localhost:3000/channels/${channelId}/messages`)
